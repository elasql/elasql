...
public class ConservativeOrderedLockMonitor {
	...
	private static List<Map.Entry<Integer, Queue<Long>>> hot_history = new LinkedList<Map.Entry<Integer, Queue<Long>>>();
	...
	public static void addHotLatchHistory(int length, Queue<Long> txs) {
		System.out.println("A history is added: " + length + txs.toString());
		Entry<Integer, Queue<Long>> entry = new SimpleEntry<Integer, Queue<Long>>(length, txs);
		hot_history.add(entry);
	}
	...
	public static List<Map.Entry<Integer, Queue<Long>>> getHotLatchHistory(){
		// Return a new copy to prevent being modified after being got.
		return new LinkedList<Map.Entry<Integer, Queue<Long>>>(hot_history);
	}
}

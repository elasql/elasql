...
public class TransactionDiskIORecorder extends Task {
	private static Logger logger = Logger.getLogger(TransactionDiskIORecorder.class.getName());

	private final static String THREAD_NAME = "Transaction Disk I/O Count Recorder";
	private final static String FILENAME_PREFIX = "transaction-disk";
	private final static String TRANSACTION_ID_COLUMN = "Transaction ID";
	private final static String FIRST_STATS_COLUMN = "Total I/O";
	...
	public static void startRecording() {
		if (!isRecording.getAndSet(true)) {
			...
			VanillaDb.taskMgr().runTask(new TransactionDiskIORecorder());
		}
	}
	
	public static void recordResult(long txNum, DiskIOCounter counter) {
		if (!isRecording.get())
			return;
			
		TransactionStatistics stats = new TransactionStatistics(txNum);
		for (Object component : counter.getComponents())
			stats.addRecord(component.toString(), counter.getComponentIOCount(component));
		queue.add(stats);
	}
	...
	public void run() {
		...
		try {
			...
			if (logger.isLoggable(Level.INFO)) {
				String log = String.format("No more disk io informations coming in last %d seconds. Start generating a report.",
						TIME_TO_FLUSH);
				logger.info(log);
			}

...
public class TransactionMetricRecorder extends Task {
	...
	private void saveToCsv() {
		...
		saveToLatchHotHistory(ConservativeOrderedLockMonitor.getHotLatchHistory());
	}
	...
	private void saveToLatchHotHistory(List<Map.Entry<Integer, Queue<Long>>> history) {
		String fileName = String.format("%s-latch-hot-history-server-%d", FILENAME_PREFIX, serverId);
		try {
		      FileWriter writer = new FileWriter(fileName + ".csv");
		      StringBuilder sb = new StringBuilder();
		      ...
		      sb.append("Queue_Txs");
		      sb.append('\n');
		      for(Entry<Integer, Queue<Long>> record: history) {
		    	  if(record != null) {
		      ...
		      writer.close();
		      System.out.println("Successfully wrote latch hot history to the file.");
		} catch (IOException e) {
		      System.out.println("An error occurred.");
		      e.printStackTrace();
			}

...
public class FeatureExtractor {
	...
	private TransactionDependencyAnalyzer dependencyAnalyzer = new TransactionDependencyAnalyzer();

	private TpartMetricWarehouse metricWarehouse;

	Map<String, Integer> tableIndexMap = new HashMap<String, Integer>();

	public FeatureExtractor(TpartMetricWarehouse metricWarehouse) {
		this.metricWarehouse = metricWarehouse;
		tableIndexMap.put("customer", 0);
		tableIndexMap.put("district", 1);
		tableIndexMap.put("history", 2);
		tableIndexMap.put("item", 3);
		tableIndexMap.put("stock", 4);
		tableIndexMap.put("warehouse", 5);
		tableIndexMap.put("order_line", 6);
		tableIndexMap.put("orders", 7);
		tableIndexMap.put("new_order", 8);
	}

	/**
	 * Extracts the features from the stored procedure and the current T-grpah. Note
	 * that if stored procedures are processed in batches, some stored procedures in
	 * front of the current one may not yet be inserted to the T-graph.
	 * 
	...
	public TransactionFeatures extractFeatures(TPartStoredProcedureTask task, TGraph graph) {
		...
		if (task.getTxNum() <= lastProcessedTxNum)
			throw new RuntimeException(String.format("Transaction requests are not passed to FeatureExtractor "
					+ "in the total order: %d, last processed tx: %d", task.getTxNum(), lastProcessedTxNum));
		...
		// New added Feature
		builder.addFeature("Read PrimaryKey", extractPrimaryKeyFeature(task.getReadSet(), graph));
		builder.addFeature("Write PrimaryKey", extractPrimaryKeyFeature(task.getWriteSet(), graph));
		builder.addFeature("Insert PrimaryKey", extractPrimaryKeyFeature(task.getInsertSet(), graph));

		// Get dependencies
		Set<Long> dependentTxs = dependencyAnalyzer.addAndGetDependency(task.getTxNum(), task.getReadSet(),
				task.getWriteSet());

...
public class TPartScheduler extends Task implements Scheduler {
	...
	public TPartScheduler(TPartStoredProcedureFactory factory, BatchNodeInserter inserter, Sinker sinker,
			TGraph graph) {
		this(factory, inserter, sinker, graph, true);
	}

	public TPartScheduler(TPartStoredProcedureFactory factory, BatchNodeInserter inserter, Sinker sinker, TGraph graph,
			boolean isBatching) {
	...
	public void run() {
		...
		while (true) {
			try {
				...
				if ((batchingEnabled && batchedTasks.size() >= SCHEDULE_BATCH_SIZE) || !batchingEnabled) {
	...
	private void printGraphStatistics() {
		...
		if (time >= nextReportTime) {
			...
			System.out.println(
					String.format("Time: %d, avg. imbal: %f, avg. remote tx reads: %f, " + "avg. remote sink reads: %f",
							time, ((double) imbalanced) / recordCount, ((double) remoteTxRead) / recordCount,
							((double) remoteSinkRead) / recordCount));
			// System.out.println("==================================\n");
			...
			// GraphDumper.dumpToFile(new File(dumpDir, String.format("%d_%d.txt", time,
			// batchId)), graph);
		}
		// }
		batchId++;
	...
	private void collectGraphStatistics() {
		...
		if (time >= nextReportTime) {
			...
			// # of transactions
			System.out.println(String.format("Number of transactions: %s.", Arrays.toString(numberOfTxns)));
			Arrays.fill(numberOfTxns, 0);

			// # of dist transactions
			System.out.println(String.format("Number of dist. transactions: %s.", Arrays.toString(numberOfDistTxns)));
			Arrays.fill(numberOfDistTxns, 0);

			// % of dist transactions
			System.out.println(String.format("Ratio of dist. transactions: %s.", Arrays.toString(ratioOfDistTxns)));

			// # of remote reads
			System.out.println(String.format("Number of remote reads: %s.", Arrays.toString(numberOfRemoteReads)));
	...
	private void printImbalStatistics() {
		...
		if (time >= nextReportTime) {
			...
			System.out.println(String.format("Average imbalanced: %f at time %d.", averageImb, time));

...
public class MigrationAnalysisProc extends CalvinStoredProcedure<StoredProcedureParamHelper> {
	...
	public MigrationAnalysisProc(long txNum) {
		...
		System.out.println("I am " + Elasql.serverId() + " got MigrationAnalysisProc Source "
				+ Elasql.migrationMgr().getSourcePartition() + " Dest " + Elasql.migrationMgr().getDestPartition()
				+ " at " + ((System.currentTimeMillis() - MigrationManager.startTime) / 1000));
	}
	...
	public void prepareKeys() {
		...
		if (Elasql.serverId() == sourceNode) {
			...
			if (logger.isLoggable(Level.INFO))
				logger.info("Estimated size of target data set for migration: " + dataSet.size());
